#!/usr/bin/env bash
# One-time bootstrap for the replacement Virginia ROOT host.
# Run as root: sudo bash bootstrap-root-virginia.sh root.nexinus.net
set -Eeuo pipefail

DOMAIN="${1:?Usage: sudo bash bootstrap-root-virginia.sh <root-domain>}"
ROOT_DEPLOY_PUBLIC_KEY="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEJ11WblnHTfkik8EmRAljsh+qyDZg5wBDdKR8MbYZrw root-gate-deploy-oregon-2026"
APP_USER="rootapp"
DEPLOY_USER="rootdeploy"
...
id -u "${DEPLOY_USER}" >/dev/null 2>&1 || useradd --create-home --home-dir "/home/${DEPLOY_USER}" --shell /bin/bash "${DEPLOY_USER}"
install -d -m 0700 -o "${DEPLOY_USER}" -g "${DEPLOY_USER}" "/home/${DEPLOY_USER}/.ssh" "/home/${DEPLOY_USER}/releases"
printf '%s\n' "${ROOT_DEPLOY_PUBLIC_KEY}" > "/home/${DEPLOY_USER}/.ssh/authorized_keys"
...
echo "ROOT Virginia bootstrap complete. A fresh production encryption key was generated locally on this host."
