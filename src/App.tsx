...
import { ecosystemConnections } from "./ecosystem";

type View = "home" | "reclaim" | "member" | "steward" | "truth" | "services" | "proof" | "emergency" | "ecosystem";
...
type LedgerKind = "account_control" | "attestation" | "consent" | "consent_grant" | "claim" | "correction" | "selective_disclosure_draft";
...
type VouchItem = { id: string; direction: "issued" | "received"; counterpartyHandle: string; scope: string; statement: string; strength: number; state: "active" | "withdrawn"; integrityDigest: string; privacy: "member_to_member_private"; credential: "not_issued" };
...
function App() {
  ...
  const [vouchScope, setVouchScope] = useState("specific_interaction");
  const [vouchStrength, setVouchStrength] = useState("3");
  const [vouchStatement, setVouchStatement] = useState("");
  const [presentationOrigin, setPresentationOrigin] = useState("https://xinus.one");
  const [presentationRecipient, setPresentationRecipient] = useState("XINUS MonarchOS");
  const [presentationPurpose, setPresentationPurpose] = useState("");
  const [presentationChallenge, setPresentationChallenge] = useState("");
  const [presentationClaims, setPresentationClaims] = useState<string[]>(["identity_posture"]);
  ...
  const requestVouches = async (method: "GET" | "POST", path = "/api/vouches", body?: unknown) => {
    const response = await fetch(path, { method, headers: body ? { "content-type": "application/json" } : undefined, credentials: "same-origin", body: body ? JSON.stringify(body) : undefined });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || "ROOT could not update your private vouches.");
    setVouches(payload.items);
  };
  ...
  useEffect(() => {
    if (!account) { setPathway([]); setLedger([]); setVouches([]); setIdentity(null); setPathwayReady(true); setLedgerReady(true); setIdentityReady(true); return; }
    setPathwayReady(false);
    setLedgerReady(false);
    setIdentityReady(false);
    Promise.all([requestPathway("GET"), requestLedger("GET"), requestVouches("GET"), loadIdentity()])
  ...
  const changeLedgerState = async (record: LedgerItem) => {
    const action = ["consent", "consent_grant", "selective_disclosure_draft"].includes(record.kind) ? "revoke" : "withdraw";
    try {
      await requestLedger("POST", `/api/ledger/${encodeURIComponent(record.id)}/${action}`);
      setNotice(["consent", "consent_grant", "selective_disclosure_draft"].includes(record.kind) ? "ROOT revoked that private consent or presentation record. No external access existed to revoke." : "ROOT withdrew that private record and created a new integrity state.");
  ...
  const createVouch = async () => {
    try {
      await requestVouches("POST", "/api/vouches", { recipientHandle: vouchRecipient, scope: vouchScope, strength: Number(vouchStrength), statement: vouchStatement });
      setVouchRecipient(""); setVouchStatement(""); setVouchStrength("3");
      setNotice("ROOT recorded a private member-to-member vouch. It is visible only to the two members, carries no public rating, and issues no credential.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not record that private vouch."); }
  };

  const withdrawVouch = async (vouchId: string) => {
    try {
      await requestVouches("POST", `/api/vouches/${encodeURIComponent(vouchId)}/withdraw`);
      setNotice("ROOT marked your private vouch withdrawn. Both members retain the private historical state while their accounts exist.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not withdraw that private vouch."); }
  };

  const togglePresentationClaim = (claim: string) => setPresentationClaims(current => current.includes(claim) ? current.filter(item => item !== claim) : current.length >= 3 ? current : [...current, claim]);

  const createPresentationDraft = async () => {
    try {
      await requestLedger("POST", "/api/ledger/presentation-drafts", { recipientOrigin: presentationOrigin, recipientLabel: presentationRecipient, purpose: presentationPurpose, requestedClaims: presentationClaims, challenge: presentationChallenge, expiresAt: Date.now() + 5 * 60 * 1000 });
      setPresentationPurpose(""); setPresentationChallenge("");
      setNotice("ROOT recorded a five-minute selective-disclosure draft. It did not issue a credential, sign a presentation, contact the recipient, or transfer any data.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not record that presentation draft."); }
  };
  ...
  const vouchControls = account && <section className="page sovereign-controls"><p className="eyebrow">PRIVATE MEMBER VOUCHES</p><h2>Vouch without creating a public score.</h2><p className="lede">A vouch is a private statement from one ROOT member to another. It is not a credential, rating, universal truth score, or verification of real-world facts. Only the issuing and receiving members can see it.</p><div className="member-grid identity-grid"><article><p className="eyebrow">ISSUE A PRIVATE VOUCH</p><label>Recipient ROOT handle<input value={vouchRecipient} onChange={event => setVouchRecipient(event.target.value)} placeholder="member-handle" /></label><label>Scope<select value={vouchScope} onChange={event => setVouchScope(event.target.value)}><option value="account_control">Account control</option><option value="specific_interaction">Specific interaction</option><option value="member_stated_collaboration">Member-stated collaboration</option></select></label><label>Strength<select value={vouchStrength} onChange={event => setVouchStrength(event.target.value)}>{[1, 2, 3, 4, 5].map(value => <option key={value} value={String(value)}>{value} of 5</option>)}</select></label><label>Private statement<textarea value={vouchStatement} onChange={event => setVouchStatement(event.target.value)} placeholder="State only what you personally choose to support." /></label><button className="pill" onClick={createVouch}>Record private vouch</button></article><article><p className="eyebrow">MY ISSUED & RECEIVED VOUCHES</p><h3>Two-member visibility only.</h3>{vouches.length === 0 ? <p>No private vouches yet.</p> : vouches.map(vouch => <div className="ledger-record" key={vouch.id}><p className="tag">{vouch.direction} · {vouch.state} · strength {vouch.strength}/5</p><b>{vouch.counterpartyHandle}</b><span>{vouch.statement}</span><small>{vouch.scope.replaceAll("_", " ")} · integrity {vouch.integrityDigest.slice(0, 18)}…</small>{vouch.direction === "issued" && vouch.state === "active" && <button className="text danger" onClick={() => withdrawVouch(vouch.id)}>Withdraw my vouch</button>}</div>)}</article></div></section>;

  const disclosureControls = account && <section className="page sovereign-controls"><p className="eyebrow">SELECTIVE-DISCLOSURE DRAFT</p><h2>Prepare a narrow future presentation without sending it.</h2><p className="lede">ROOT creates only a private draft tied to one HTTPS recipient, purpose, challenge, claim selection, short expiry, integrity receipt, and revocation. It does not issue a credential, contact the recipient, or transfer a claim.</p><div className="member-grid identity-grid"><article><label>Recipient HTTPS origin<input value={presentationOrigin} onChange={event => setPresentationOrigin(event.target.value)} placeholder="https://recipient.example" /></label><label>Recipient label<input value={presentationRecipient} onChange={event => setPresentationRecipient(event.target.value)} /></label><label>Single purpose<textarea value={presentationPurpose} onChange={event => setPresentationPurpose(event.target.value)} placeholder="Why I might later present selected ROOT facts." /></label><label>Verifier challenge<input value={presentationChallenge} onChange={event => setPresentationChallenge(event.target.value)} placeholder="Verifier-provided URL-safe nonce" /></label><div className="scope-list">{[["identity_posture", "Identity posture"], ["account_control_receipt", "Account-control receipt"], ["attestation_integrity_receipt", "Attestation integrity receipt"], ["vouch_summary", "Vouch summary (not individual vouches)"]].map(([claim, label]) => <label key={claim}><input type="checkbox" checked={presentationClaims.includes(claim)} onChange={() => togglePresentationClaim(claim)} /> {label}</label>)}</div><button className="pill" onClick={createPresentationDraft}>Record five-minute presentation draft</button></article><article><p className="eyebrow">IMPLEMENTATION STATUS</p><h3>Draft-only until every gate is met.</h3><p>ROOT does not yet issue DIDs, Verifiable Credentials, signed presentations, status-list entries, or third-party verifier calls. A future execution path requires a signed recipient manifest, key rotation, replay protection, security review, retention limits, region disclosure, and a member approval immediately before one-time delivery.</p><a className="line" href="/selective-disclosure-presentation-spec-v0.1.md" target="_blank" rel="noreferrer">Read the public technical specification ↗</a>{ledger.filter(record => record.kind === "selective_disclosure_draft").map(record => <div className="grant-state" key={record.id}><b>{String(record.payload.recipientLabel)}</b><span>{record.state.replaceAll("_", " ")}</span>{record.state === "recorded_not_executed" && <button className="text danger" onClick={() => changeLedgerState(record)}>Revoke draft</button>}</div>)}</article></div></section>;

  return <div className="app">
    <header>
      <button className="brand" onClick={() => setView("home")}><span>R</span><b>ROOT <small>SOVEREIGN GATEWAY</small></b></button>
      <nav><button onClick={() => setView("home")}>Gateway</button><button onClick={() => setView("reclaim")}>Project Reclaim</button><button onClick={() => setView("services")}>Connection center</button><button onClick={() => setView("ecosystem")}>Ecosystem</button><button onClick={() => setView("member")}>Member space</button></nav>
    ...
    {view === "member" && account && vouchControls}
    {view === "member" && account && disclosureControls}
    ...
    {view === "ecosystem" && <main className="page"><p className="eyebrow">ECOSYSTEM CONNECTION REGISTRY</p><h1>Connected by choice, not hidden data flow.</h1><p className="lede">ROOT lists known ecosystem surfaces and supplied infrastructure as explicit connection records. A link is not a trust grant; a supplied instance is not an enrolled node; and no connection carries a ROOT account, profile, session, private proof, emergency context, or Project Reclaim member record automatically.</p><section className="cards">{ecosystemConnections.map(connection => <article key={connection.id}><p className="tag">{connection.connectionState.replaceAll("_", " ")} · {connection.region}</p><h2>{connection.label}</h2><p><b>Role:</b> {connection.role}</p><p>{connection.boundary}</p>{connection.endpoint ? <a href={connection.endpoint} target="_blank" rel="noreferrer">Open by choice ↗</a> : <small>Canonical HTTPS endpoint pending owner review. ROOT does not contact this instance.</small>}</article>)}</section><section className="member-grid"><article><h2>What must happen before deeper connection</h2><p>Each application needs controlled HTTPS, owner-approved deployment access, a data and telemetry map, signed public manifest, signing-key rotation and revocation process, defined retention and processing region, allowlisted origins, and member-visible purpose-specific consent. Until then, ROOT remains link-only.</p></article><article><h2>Public registry</h2><p>The public registry deliberately lists no IP addresses, keys, health metrics, member identifiers, or node enrollment claims. It declares only link-only and endpoint-pending state.</p><a className="pill" href="/ecosystem-connection-registry-v0.1.json" target="_blank" rel="noreferrer">View public registry JSON ↗</a></article></section></main>}
