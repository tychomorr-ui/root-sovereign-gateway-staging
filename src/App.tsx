...
import { needDescriptions, publicSources, type Need, type Source } from "./policy";
...
function App() {
  ...
  const matchingSources = selectedNeed === "All" ? sources : sources.filter(source => source.category === selectedNeed);
  ...
  const requestPathway = async (method: "GET" | "POST" | "PATCH" | "DELETE", body?: unknown, sourceId?: string) => {
    const response = await fetch(sourceId ? `/api/pathway/${encodeURIComponent(sourceId)}` : "/api/pathway", {
      method, headers: body ? { "content-type": "application/json" } : undefined, credentials: "same-origin", body: body ? JSON.stringify(body) : undefined,
    });
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/auth/me", { credentials: "same-origin" }).then(async response => response.ok ? response.json() : null).then(payload => {
      if (!active) return;
  ...
  const submitAuth = async (mode: "register" | "login") => {
    ...
    try {
      ...
      setAccount(payload.user); setPassword("");
  ...
  const signOut = async () => { await fetch("/api/auth/logout", { method: "POST", credentials: "same-origin" }).catch(() => undefined); setAccount(null); setAuthMessage("Your ROOT session has ended on this device."); };
  const deleteAccount = async () => {
    ...
    try {
      ...
      const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Account deletion could not be completed.");
      setAccount(null); setPathway([]); setAuthMessage("Your local ROOT account, private action plan, and active sessions were deleted.");
  ...
  const save = async (source: Source) => {
    ...
    try { await requestPathway("POST", { sourceId: source.id }); setView("member"); setNotice(`Saved “${source.title}” to your private action plan. ROOT has not contacted anyone.`); }
    catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not save that action-plan step."); }
  };
  const updateStage = async (sourceId: string, stage: PathwayItem["stage"]) => { try { await requestPathway("PATCH", { sourceId, stage }); } catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not update that step."); } };
  const remove = async (sourceId: string) => { try { await requestPathway("DELETE", undefined, sourceId); } catch (error) { setNotice(error instanceof Error ? error.message : "ROOT could not remove that step."); } };

  const navigator = <section className="navigator" aria-label="Mendocino County connection navigator">
    <div><p className="eyebrow">CONNECTION NAVIGATOR</p><h2>Choose the kind of help you want to explore.</h2><p>ROOT filters reviewed public pathways by your choice. It does not score you, send your details, or decide eligibility.</p></div>
    <div className="need-chips"><button className={selectedNeed === "All" ? "active" : ""} onClick={() => setSelectedNeed("All")}>All pathways</button>{(Object.keys(needDescriptions) as Need[]).map(need => <button className={selectedNeed === need ? "active" : ""} key={need} onClick={() => setSelectedNeed(need)}>{need}</button>)}</div>
    {selectedNeed !== "All" && <p className="selection-note"><b>{selectedNeed}:</b> {needDescriptions[selectedNeed]}</p>}
  </section>;

  return <div className="app">
    <header><button className="brand" onClick={() => setView("home")}><span>R</span><b>ROOT <small>SOVEREIGN GATEWAY</small></b></button><nav><button onClick={() => setView("home")}>Gateway</button><button onClick={() => setView("reclaim")}>Project Reclaim</button><button onClick={() => setView("services")}>Connection center</button><button onClick={() => setView("member")}>Member space</button></nav><button className="pill" onClick={() => setView("member")}>{account ? `${account.handle} ↗` : "Open ROOT ↗"}</button></header>

    {view === "home" && <main><section className="hero"><p className="eyebrow">PRIVATE BY DEFAULT</p><h1>ROOT is the gateway.<br /><i>Take the next step on your terms.</i></h1><p className="lede">ROOT organizes reviewed sources into an action plan you control. It never scores your life, files an application, contacts a provider, or shares your pathway without a separate grant.</p><div className="actions"><button className="pill" onClick={() => setView("services")}>Find a pathway</button><button className="line" onClick={() => setView("member")}>Open my private plan</button></div></section><section className="principles"><article><b>Choose</b><span>Pick a need without creating a public profile.</span></article><article><b>Prepare</b><span>Save official next steps in your private plan.</span></article><article><b>Decide</b><span>You choose whether to call, apply, or leave ROOT.</span></article><article><b>Revoke</b><span>No external access exists unless you explicitly grant it.</span></article></section><section className="control-surface"><p className="eyebrow">ROOT CONTROL SURFACE</p><h2>Connection without capture.</h2><div><button onClick={() => setView("services")}><b>Connection center</b><span>Reviewed local pathways</span></button><button onClick={() => setView("member")}><b>My action plan</b><span>Private steps you control</span></button><button onClick={() => setView("emergency")}><b>Urgent support</b><span>Clear crisis boundaries</span></button><button onClick={() => setView("proof")}><b>Proof ledger</b><span>Consent receipts, not scores</span></button></div></section><section className="connections"><p className="eyebrow">ECOSYSTEM HANDOFFS</p><h2>Open a destination by choice.</h2><p>These links do not carry your ROOT profile, permissions, session, or action-plan data.</p><div>{links.map(([label, url]) => <a key={url} href={url} target="_blank" rel="noreferrer"><b>{label} ↗</b><small>Link only · no ROOT data transfer</small></a>)}</div></section></main>}

    {view === "reclaim" && <main className="page"><p className="eyebrow">PROJECT RECLAIM</p><h1>Move from a need to a next step.</h1><p className="lede">Project Reclaim starts with Mendocino County sources. ROOT can help you organize options; official providers decide availability, eligibility, applications, and outcomes.</p><section className="boundary"><div><b>Mendocino County</b><span>Reviewed public pathways and a private member action plan.</span></div><div><b>Lake County</b><span>Verified watchlist only — not published in ROOT.</span></div></section>{navigator}<div className="cards">{matchingSources.map(source => <article key={source.id}><p className="tag">{source.category} · Mendocino County</p><h2>{source.title}</h2><p>{source.note}</p>{source.contact && <p className="contact"><b>Official contact:</b> {source.contact}</p>}<p className="next-step"><b>Next step:</b> {source.action}</p><p className="review"><b>Source review:</b> {source.sourceKind} · checked {source.verifiedAt}</p><a href={source.url} target="_blank" rel="noreferrer">Open official source ↗</a><button className="text" onClick={() => save(source)}>Add to my private plan</button></article>)}</div></main>}

    {view === "services" && <main className="page"><p className="eyebrow">CONNECTION CENTER</p><h1>More than a directory.</h1><p className="lede">Choose a category, see what the official source can do, preserve the next step privately, and only then decide whether to hand yourself off. ROOT has no provider referral agreement and sends no information on your behalf.</p>{navigator}<section className="connection-steps"><article><b>1 · Find</b><span>Filter a reviewed source by your chosen need.</span></article><article><b>2 · Plan</b><span>Save a minimal private step in ROOT.</span></article><article><b>3 · Act</b><span>You choose whether to use the official contact or web route.</span></article><article><b>4 · Reflect</b><span>Mark your own progress; no provider sees it.</span></article></section><div className="cards">{matchingSources.map(source => <article key={source.id}><p className="tag">{source.category} · REVIEWED</p><h2>{source.title}</h2><p>{source.note}</p><p className="next-step"><b>What you can do:</b> {source.action}</p>{source.contact && <p className="contact">{source.contact}</p>}<a href={source.url} target="_blank" rel="noreferrer">Open official source ↗</a><button className="text" onClick={() => save(source)}>Save my next step</button></article>)}</div></main>}

    {view === "member" && <main className="page"><p className="eyebrow">MEMBER SPACE</p><h1>My private action plan</h1><p className="lede">This plan stays inside your encrypted ROOT account. Saving a step never contacts a provider, starts an application, or shares your identity.</p>{notice && <p className="notice" role="status">{notice}</p>}<section className="member-grid">{!authReady ? <article><h2>Checking ROOT account</h2><p>ROOT is checking only its own local session service.</p></article> : !account ? <article className="account-card"><h2>Local ROOT account</h2><p>Create an account with a ROOT handle and password to retain a private action plan. ROOT does not use a social login, email identity broker, advertising identifier, or tracking SDK.</p><label>ROOT handle<input value={handle} onChange={event => setHandle(event.target.value)} autoComplete="username" placeholder="your-root-handle" /></label><label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="new-password" placeholder="14 characters minimum" /></label><div className="actions"><button className="pill" onClick={() => submitAuth("register")}>Create local account</button><button className="line" onClick={() => submitAuth("login")}>Sign in</button></div><p className="auth-message">{authMessage}</p></article> : <article><h2>{account.handle}</h2><p><b>Account authority:</b> self-owned ROOT account.</p><p><b>External identity provider:</b> none.</p><p><b>Current session:</b> opaque, HTTP-only ROOT session cookie.</p><button className="line" onClick={signOut}>Sign out</button><button className="text danger" onClick={deleteAccount}>Delete account and action plan</button><p className="auth-message">{authMessage}</p></article>}<article className="plan-card"><h2>My steps</h2>{!account ? <p>Open a local ROOT account to save a private plan. Public sources remain available without a sign-in.</p> : !pathwayReady ? <p>Loading your private plan…</p> : planSources.length === 0 ? <p>No saved steps yet. Choose a reviewed local pathway when you are ready.</p> : planSources.map(({ item, source }) => <div className="path" key={item.sourceId}><p className="tag">{source.category}</p><b>{source.title}</b><span>{source.action}</span><label>My progress<select value={item.stage} onChange={event => updateStage(item.sourceId, event.target.value as PathwayItem["stage"])}>{(Object.keys(stageLabels) as PathwayItem["stage"][]).map(stage => <option key={stage} value={stage}>{stageLabels[stage]}</option>)}</select></label><div className="path-actions"><a href={source.url} target="_blank" rel="noreferrer">Official source ↗</a><button onClick={() => remove(item.sourceId)}>Remove</button></div></div>)}<button className="pill" onClick={() => setView("services")}>Find a pathway</button></article><article><h2>What ROOT will not do</h2><p>ROOT will not submit a provider form, send a referral, expose your plan to an operator, or create a behavioral profile.</p><p>Eligibility, appointments, service availability, and outcomes belong to the official source—not ROOT.</p><button className="line" onClick={() => setView("proof")}>View permission boundary</button><button className="text" onClick={() => setView("emergency")}>Urgent-support boundary</button></article></section></main>}

    {view === "steward" && <main className="page"><p className="eyebrow">DIRECTORY STEWARD</p><h1>Clear limits, visible review.</h1><section className="member-grid"><article><h2>Authority</h2><p>Review Mendocino County public-source accuracy and the date each official source was checked.</p><p>Excluded: member plans, operator roster, Monarch nodes, attestations, provider decisions, and Lake County publication.</p></article><article><h2>Lake County gate</h2><p>Lake County remains watchlist-only until a separately authorized publication scope exists.</p><button disabled>Publish Lake County source</button></article></section></main>}
    ...
    {view === "emergency" && <main className="page"><p className="eyebrow">URGENT SUPPORT BOUNDARY</p><h1>Clear choices in a hard moment.</h1><p className="lede">ROOT does not dispatch emergency services, monitor location, or retain a crisis history. It can show verified public contact paths so you decide what to do next.</p><section className="urgent-grid"><article className="urgent"><p className="tag">LIFE-THREATENING EMERGENCY</p><h2>Call 911</h2><p>If there is immediate danger or a life-threatening emergency, use local emergency services.</p></article><article><p className="tag">24/7 MENTAL-HEALTH CRISIS</p><h2>County Crisis Line</h2><p>Mendocino County lists this as a 24/7 crisis line for immediate mental-health help.</p><p className="contact"><b>1-855-838-0404</b></p><a href="https://www.mendocinocounty.gov/residents/health/mental-health" target="_blank" rel="noreferrer">Official county details ↗</a></article><article><p className="tag">NON-CRISIS EMOTIONAL SUPPORT</p><h2>County Warm Line</h2><p>For stress, isolation, overwhelm, or non-crisis support; it is not emergency dispatch.</p><p className="contact"><b>707-472-2311 · 1-833-955-2510</b></p><a href="https://www.mendocinocounty.gov/departments/behavioral-health-and-recovery-services" target="_blank" rel="noreferrer">Official county details ↗</a></article></section></main>}
