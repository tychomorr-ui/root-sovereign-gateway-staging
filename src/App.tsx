...
type LedgerKind = "account_control" | "attestation" | "consent" | "claim" | "correction";
type LedgerState = "active" | "corrected" | "withdrawn" | "revoked";
...
function App() {
  ...
  useEffect(() => {
    if (!account) { setPathway([]); setLedger([]); setPathwayReady(true); setLedgerReady(true); return; }
    setPathwayReady(false);
    setLedgerReady(false);
    Promise.all([requestPathway("GET"), requestLedger("GET")])
      .catch(error => setNotice(error instanceof Error ? error.message : "ROOT could not load your private member records."))
      .finally(() => { setPathwayReady(true); setLedgerReady(true); });
  }, [account?.id]);
  ...
  const changeLedgerState = async (record: LedgerItem) => {
    const action = record.kind === "consent" ? "revoke" : "withdraw";
    try {
      await requestLedger("POST", `/api/ledger/${encodeURIComponent(record.id)}/${action}`);
      setNotice(record.kind === "consent" ? "ROOT revoked that private consent receipt. No external access existed to revoke." : "ROOT withdrew that private record and created a new integrity state.");
