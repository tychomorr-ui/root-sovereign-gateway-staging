...
## Virginia Host Migration and Private Vouch Release

The former Oregon ROOT-Gate instance was deleted by the owner before this release. The replacement host is `ROOT_SOVEREIGN-IDENTITY` in AWS Lightsail Virginia (`us-east-1a`), using the static IPv4 address `34.198.19.147`. DNS for `root.nexinus.net` was redirected to that address. Because the Oregon encrypted account store was deleted with its host, this migration is a **fresh production store** and does not claim recovery of any previous account or private-record data.

The Virginia bootstrap created a fresh root-owned production encryption key locally, created the least-privilege `rootapp` runtime user and `rootdeploy` release user, authorized only the dedicated ROOT deployment public key, installed the root-owned release activator, configured Caddy as the HTTPS reverse proxy, and enabled host firewall access for 22, 80, and 443 only. The application service is bound only to `127.0.0.1:4174`; a direct public probe of port 4174 was closed. Public and loopback requests to `/api/auth/me` both returned HTTP 200 with the expected anonymous-session response through the Virginia deployment.

Release `20260815T131442Z-f27b308` adds private member-to-member vouches, selective-disclosure presentation drafts, export v3, and the reviewed ecosystem connection registry. A vouch has a local ROOT recipient handle, constrained scope, strength, statement, integrity receipt, issuer-owned withdrawal, and an explicit `not_issued` credential state. It is visible only to its issuer and recipient; it is not a public rating, score, credential, or third-party proof. Selective-disclosure drafts bind a chosen claim list, recipient origin and label, purpose, challenge, and expiry to a member-controlled, revocable `recorded_not_executed` record. ROOT does not transfer data, mint a DID or W3C Verifiable Credential, sign an external presentation, or contact the named recipient.

The release passed 27 automated tests, TypeScript validation, and a production build locally and again on the Virginia host through the root-owned installer. Disposable production verification created three fresh accounts, issued one private vouch, confirmed an unrelated member could not read it, confirmed its intended recipient could read it, denied recipient withdrawal, created and revoked a selective-disclosure draft, withdrew the vouch as its issuer, verified its private v3 export history and `not_issued` interoperability boundary, deleted the issuer, confirmed the recipient no longer saw the deleted issuer’s vouch, and deleted all disposable accounts. No existing member data was inspected or modified during this verification.

The public ecosystem registry is a transparent link-only map for the supplied applications and hosts. Each future cross-application connection remains endpoint-pending and does not carry a ROOT profile, session, permissions, vouch, private record, location, or action-plan data. A future integration requires a separately approved recipient endpoint, member-selected disclosure scope, challenge, expiration, one-time presentation state, revocation handling, and a technical verifier review.
