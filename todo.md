...
- [x] Expand ROOT’s source model from a Mendocino-only directory to source-governed multi-county public resource packs, while keeping Lake County unpublished until separately authorized.
- [x] Design future Monarch OS mesh synchronization to distribute only signed, approved public resource packs and Project Reclaim materials—never private member plans, account data, emergency context, or provider records.
- [x] Elevate Project Reclaim into ROOT’s flagship restoration, opportunity, training, evidence, and participation architecture using the user-provided initiative directive without fabricating projects, jobs, inventory, partners, or outcomes.
- [x] Add source-verification, status, provenance, and explicit publication-state controls for any future Project Reclaim project, opportunity, material, or regional record.
- [x] Add a reusable county configuration model that represents Mendocino as published, Lake as watchlist-only, and future counties as explicit unpublished configurations until approved.
...
- [x] Research and publish only verified Lake County public resource pathways under the user’s newly authorized regional scope, with source review and no implied provider relationship.
- [x] Add empty, source-governed Project Reclaim registry schemas for projects, jobs, materials, partners, metrics, evidence, signatures, CIDs, and ledger references; do not invent operational records.
...
- [x] Publish NEXINUS RI Systems LLC as the owner-authorized initiating organization for Project Reclaim in Mendocino and Lake Counties without storing or exposing the EIN.
- [x] Publish a Project Reclaim material-recovery intake opportunity for Mendocino and Lake Counties that accepts real source-backed listings but does not claim available inventory, public sites, or confirmed partners.
- [x] Add tests, release, and live verification for the first owner-authorized organization and material-recovery intake records with CID, signature, ledger, and map-location fields honestly pending.
...
- [x] Deliver a second ROOT-centered compound upgrade for sovereign identity infrastructure: member profile, credentials, consent grants, recovery, portability, provenance, correction, revocation, and operational transparency.
- [x] Audit the current ROOT identity infrastructure and document every capability that is live, missing, explicitly not offered, or deferred by member-safety constraints.
- [x] Implement, test, release, and verify the highest-leverage sovereign identity upgrades without adding external identity brokers, tracking, fabricated credentials, or automatic third-party data exchange.
- [x] Add an encrypted member identity profile with an optional private display name, self-descriptions, and member-selected identity posture; do not present it as a legal or third-party-verified identity.
- [x] Add one-time recovery-kit generation, rotation, and revocation plus member-visible session management without email recovery, social login, or external recovery custodians.
- [x] Add explicit private consent grants with recipient, purpose, data scope, expiration, and revocation fields; do not perform third-party data transfer or represent a grant as externally fulfilled.
- [x] Add member-only portable JSON export with a clear integrity/credential interoperability posture and clearly state that no DID, W3C Verifiable Credential, signature, status list, or verifier integration is yet issued.
- [x] Add sovereign identity posture, recovery, session, consent, and portability controls to Member Space; add comprehensive ownership and recovery tests; release and verify the completed capability.
- [x] Publish a reviewed ecosystem connection registry for the supplied applications and instances that provides link-only handoffs, declared purpose, owner-visible endpoint metadata, and no automatic cross-application data exchange.
- [x] Define and implement private member-to-member vouches with an explicit recipient ROOT handle, a constrained statement, declared scope, strength, integrity receipt, member-visible records on both sides, and withdrawal by the voucher without turning vouches into public ratings or credentials.
- [x] Prevent a member from creating a vouch for themselves, prevent any third party from reading individual vouch content, retain withdrawn vouches as member-visible historical records, and ensure account deletion removes vouches written and received by that account.
- [x] Draft a selective-disclosure presentation protocol specifying member-selected fields, recipient binding, purpose, nonce/challenge, expiration, integrity receipt, one-time presentation state, revocation, and an explicit no-transfer/no-DID/no-VC execution boundary until an approved interoperable verifier path exists.
- [x] Add Member Space controls and ownership-focused tests for vouches, selective-disclosure draft records, and ecosystem handoff boundaries; test, release, and verify the completed capability without tracking, provider fabrication, or automatic data sharing.
- [x] Verify that `root.nexinus.net` is intentionally redirected to the Virginia ROOT_SOVEREIGN-IDENTITY instance and confirm that the owner controls its DNS and HTTPS endpoint before activation.
- [x] Establish a fresh dedicated ROOT production encryption key and least-privilege deployment account on the Virginia host; do not reuse the deleted Oregon host’s encrypted account-store key unless a secure owner-held backup is deliberately restored.
- [x] Bootstrap, deploy, and verify the tested private-vouch, selective-disclosure, and ecosystem-registry release on the Virginia host with only 22/80/443 publicly reachable and the application loopback-bound.
- [x] Record the production-host migration, live privacy verification, and any confirmed account-store recovery limitation without claiming that deleted data was recovered.
